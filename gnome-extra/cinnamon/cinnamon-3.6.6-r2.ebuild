...
src_prepare() {
	...
	eapply "${FILESDIR}"/${PN}-3.6.6-optional-networkmanager.patch
	...
	if ! use networkmanager; then
		...
		sed -i 's/;nm-applet;/;/g' files/usr/share/cinnamon-session/sessions/cinnamon*.session || die
		sed -i '/nm-applet/d' js/ui/statusIconDispatcher.js || die
	fi
